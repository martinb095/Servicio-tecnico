...
class PresupuestoController {
    ...
    public async getOne(req: Request, res: Response) {   
        pool.query('SELECT p.PkPresupuesto, p.FKCliente, c.Nombre, c.Apellido, p.FechaVigencia, p.FechaCreacion, p.Observacion, p.Confirmado from presupuesto p left join cliente c on c.PkCliente = p.FkCliente where PkPresupuesto=?;', req.params.PkPresupuesto, (err: any, results: any) => {
            if (err) {
                res.status(404).json({ text: "presup no encontrado" });
            }
            if (results) {
